...
<head>
  ...
  <style>
    .loader{height:1rem;border-radius:10px;background:#e5e7eb;width:100%;}
...
<div class="min-h-screen lg:flex">
  ...
  <aside class="bg-white dark:bg-slate-800 w-full lg:w-64 border-r border-slate-200 dark:border-slate-700 p-4 flex flex-col justify-between">
    ...
    <div>
      ...
      <nav class="space-y-2">
        <a href="/pages/dashboard" class="bg-slate-100 dark:bg-slate-700 flex items-center gap-3 p-2 rounded hover:bg-slate-100 dark:hover:bg-slate-700">
...
(async () => {
  ...
  document.getElementById('kpiTotal').querySelector('div:last-child').textContent = `Ar ${Number(total_expense.total_expense).toLocaleString('en-US', {minimumFractionDigits: 2, maximumFractionDigits: 2})}`;
  ...
  purchases.forEach(p => {
    ...
    const purchaseDate = new Date(p.purchase_date.split(' ')[0]);
    ...
    if (diffDays === 0) {
      ...
      day = purchaseDate.toLocaleDateString(undefined, {
        year: 'numeric',
        month: 'short',
        day: 'numeric'
      });
