...
$filter_by_product = $method === 'GET' && isset($_GET["product_id"]);
...
} elseif ($filter_by_product) { # /api/purchases?product_id=X
  require_once 'filter-by-product.php';

} elseif ($filter_by_date && !$get_total_expense_by_date && !$get_total_expense_by_date_range && !$get_total_expense && !$filter_by_date_range && !$filter_by_product) { # /api/purchases?purchase_date=dd-mm-yyyy
  require_once 'filter-by-date.php';

}  elseif ($filter_by_date_range && !$get_total_expense_by_date && !$get_total_expense_by_date_range && !$get_total_expense && !$filter_by_product) { # /api/purchases?start_date=dd-mm-yyyy&end_date=dd-mm-yyyy
  require_once 'filter-by-date-range.php';

} elseif ($get_total_expense_by_date && !$get_total_expense_by_date_range && !$get_total_expense && !$filter_by_date_range && !$filter_by_product) { # /api/purchases?purchase_date=dd-mm-yyyy&get_total_expense=1
  require_once 'get-total-expense-by-date.php';

} elseif($get_total_expense_by_date_range && !$get_total_expense && !$filter_by_product) { # /api/purchases?start_date=dd-mm-yyyy&end_date=dd-mm-yyyy&get_total_expense=1
  require_once 'get-total-expense-by-date-range.php';

} elseif ($get_total_expense && !$get_total_expense_by_product && !$get_total_expense_by_date && !$get_total_expense_by_date_range && !$filter_by_date_range && !$filter_by_product) { # /api/purchases?get_total_expense=1  --GET
